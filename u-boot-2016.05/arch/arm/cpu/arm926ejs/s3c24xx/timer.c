...
int timer_init(void)
{
	struct s3c24xx_timers *timers = s3c24xx_get_base_timers();
	ulong tmr;

	/* use PWM Timer 4 because it has no output */
	/* prescaler for Timer 4 is 16 */
	writel(0x0200, &timers->tcfg0);
	writel(~(0xF << 16), &timers->tcfg1);
	if (gd->arch.tbu == 0) {
		/*
		 ...
		 */
		gd->arch.tbu = 1289;//get_PCLK() / (2 * 33 * 100);
		gd->arch.timer_rate_hz = get_PCLK() / ((2+1) * 2);
...
{
	struct s3c24xx_watchdog *watchdog;

	watchdog = s3c24xx_get_base_watchdog();
