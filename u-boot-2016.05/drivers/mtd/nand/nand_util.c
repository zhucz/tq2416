...
int nand_erase_opts(nand_info_t *meminfo, const nand_erase_options_t *opts)
{
...
#if 1
	int (*nand_block_bad_old)(struct mtd_info *, loff_t , int) = NULL;//add by zhuchengzhi
#endif
	...
	erase_length = lldiv(opts->length + meminfo->erasesize - 1,meminfo->erasesize);
...
//	printf("%s : %d erase.len = %x \n",__func__,__LINE__,erase.len);
//	printf("%s : %d opts->offset = %x \n",__func__,__LINE__,opts->offset);
//	printf("%s : %d erase.addr = %x \n",__func__,__LINE__,erase.addr);
//	printf("%s : %d opts->scrub = %x \n",__func__,__LINE__,opts->scrub);
	if (opts->scrub) {
		erase.scrub = opts->scrub;
#if 0
		nand_block_bad_old = chip->block_bad;//add by zhuchengzhi
		chip->block_bad = nand_block_bad_scrub;//add by zhuchengzhi
#endif
		/*
	...
	for (erased_length = 0; erased_length < erase_length;  erase.addr += meminfo->erasesize) {
	...
	//		int ret = meminfo->_block_isbad(meminfo, erase.addr);
	...
	//				printf("%s : %d opts->quiet = %x \n",__func__,__LINE__,opts->quiet);


				if (!opts->spread)
					erased_length++;

	//				printf("%s : %d opts->spread = %x \n",__func__,__LINE__,opts->spread);
	//				printf("%s : %d erase_length = %ld \n",__func__,__LINE__,erased_length);
				continue;
	...
	//	printf("%s : %d erased_length = %ld \n",__func__,__LINE__,erased_length);
	//	printf("%s : %d mtd_erase result= %x \n",__func__,__LINE__,result);
		if (result != 0) {
	//		printf("%s : %d mtd_erase result= %x \n",__func__,__LINE__,result);
	...
	//	printf("%s : %d mtd_erase result= %x \n",__func__,__LINE__,result);
		...
		//	unsigned long long n = (unsigned long long)(erase.addr + meminfo->erasesize - opts->offset)*100;
			int percent;
	...
	if(nand_block_bad_old){
		...
		printf("%s : %d coming nand_block_bad_old \n",__func__,__LINE__);
	}
