...
extern int s3c_nand_erase(struct mtd_info *mtd, int page);
...
//	printf("%s: %d : %x\n",__func__, __LINE__,chip->read_byte(mtd));
...
static int nand_wait(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	chip->cmdfunc(mtd, NAND_CMD_STATUS, -1, -1);
 	...
 	time_start = get_timer(0);/*zhuchengzhi TODO*/
 	while (get_timer(time_start) < timer) {
//		printf("%s : %d get_timer(time_start)=%d timer = %d \n",__func__,__LINE__,get_timer(time_start),timer);
		if (chip->dev_ready) {
			if (chip->dev_ready(mtd))
				break;
		} else {
...
int nand_erase_nand(struct mtd_info *mtd, struct erase_info *instr, int allowbbt)
{
	...
	struct nand_chip *chip = mtd->priv;
//	struct nand_info *chip = mtd->priv;
	loff_t len;
	...
	while (len) {
		...
		if (!instr->scrub && nand_block_checkbad(mtd, ((loff_t) page) << chip->page_shift, 0, allowbbt)) {
		...
		printf("%s: %d  len = %ld\n:",__func__, __LINE__,len);
		printf("%s: %d  chip->pagebuf = %d\n:",__func__, __LINE__,chip->pagebuf);
		/*
		...
		printf("%s: %d  page = %d\n:",__func__, __LINE__,page);
		printf("%s: %d  chip->pagebuf = %d\n:",__func__, __LINE__,chip->pagebuf);
		printf("%s: %d  pages_per_block = %d\n:",__func__, __LINE__,pages_per_block);
		printf("%s: %d  chip->pagebuf = %d\n:",__func__, __LINE__,chip->pagebuf);
		printf("%s: %d  chip->pagemask = %d\n:",__func__, __LINE__,chip->pagemask);
		/* TODO */ 
#if 0
		if(chip->erase == NULL)
			printf("%s: %d chip->erase NULL \n:",__func__, __LINE__);
		else
			status = chip->erase(0, page & chip->pagemask);
#endif		
	...
	status = chip->erase(mtd, page & chip->pagemask);
		...
		pr_debug("%s: %d chip->erase, page 0x%08x status = %d \n",	__func__,__LINE__, page,status);
		pr_debug("%s: %d chip->erase, page 0x%08x status = %d \n",	__func__,__LINE__, (page & chip->pagemask),status);
		 ...
		 * available
		 */
		pr_debug("%s: %d chip->errstat, chip->errstat = %d \n",	__func__,__LINE__, chip->errstat);
		if ((status & NAND_STATUS_FAIL) && (chip->errstat))
			status = chip->errstat(mtd, chip, FL_ERASING, status, page);
			pr_debug("%s: %d chip->errstat, page 0x%08x status = %d \n",	__func__,__LINE__, page,status);
		...
		if (status & NAND_STATUS_FAIL) {
			...
			printf("%s: %d instr->fail_addr = %d \n",__func__, __LINE__,instr->fail_addr);
...
ident_done:
	...
	chip->badblockbits = 8;
//	chip->erase = single_erase;/*zhuchengzhi*/
	chip->erase = s3c_nand_erase;/*zhuchengzhi  TODO*/
	...
	pr_info("device found, Manufacturer ID: 0x%02x, Chip ID: 0x%02x\n", *maf_id, *dev_id);
	pr_info("from now on chip->cmdfunc = nand_command_lp \n");
...
//extern int s3c_nand_erase(struct mtd_info *mtd, int page);
//extern int s3c_nand_erase(struct mtd_info *mtd ,struct erase_info_t *erase)
...
#if 1
	mtd->_erase = nand_erase;    //zhuchengzhi
	mtd->type = MTD_NANDFLASH;
	mtd->flags = MTD_CAP_NANDFLASH;
#else
	mtd->_erase = s3c_nand_erase;    //zhuchengzhi 
	mtd->type = MTD_NANDFLASH;
	mtd->flags = MTD_CAP_NANDFLASH;
#endif 
	mtd->_read = nand_read;
