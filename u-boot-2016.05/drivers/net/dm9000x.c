...
dm9000_probe(void)
{
	...
	DM9000_DBG("%s : %d CONFIG_DM9000_BASE = 0x%x \n",__func__,__LINE__,CONFIG_DM9000_BASE);
	DM9000_DBG("%s : %d id_val = %d \n",__func__,__LINE__,id_val);
...
dm9000_reset(void)
{
	struct s3c24xx_ssmc *ssmc_reg = s3c24xx_get_base_ssmc();
	int dm_val = 0;

	dm_val = readl(&ssmc_reg->smbcr4);
	dm_val |= (1 << 0);
	DM9000_DBG("%s : %d dm_val = %x \n",__func__,__LINE__,dm_val);
	writel(dm_val,&ssmc_reg->smbsr4);
	dm_val = readl(&ssmc_reg->smbcr4);
	DM9000_DBG("%s : %d dm_val = %x \n",__func__,__LINE__,dm_val);
...
static int dm9000_init(struct eth_device *dev, bd_t *bd)
{
	...
	DM9000_DBG("DM9000: running in %x  bit mode\n",io_mode);
	switch (io_mode) {
...
static void dm9000_get_enetaddr(struct eth_device *dev)
{
	...
	char *end;
	for(i=0;i<6;i++){
		dev->enetaddr[i] = tmp?simple_strtol(tmp,&end,16):0;
		if(tmp)
			tmp = (*end)?end+1:end;
	}
#endif
...
dm9000_phy_write(int reg, u16 value)
{
	...
	DM9000_DBG("%s : %d reg:0x%x, value:0x%x\n",__func__,__LINE__,reg, value);
}
...
int dm9000_initialize(bd_t *bis)
{
	...
	gpioa_val = readl(&gpio->gpacon);
	DM9000_DBG("%s : %d gpioa_val = %x \n",__func__,__LINE__,gpioa_val);
	gpioa_val |= (1 << 15);
	writel(gpioa_val, &gpio->gpacon);//nRCS[4]
	gpioa_val = readl(&gpio->gpacon);
	DM9000_DBG("%s : %d gpioa_val = %x \n",__func__,__LINE__,gpioa_val);
