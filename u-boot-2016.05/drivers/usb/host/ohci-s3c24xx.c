...
#include <common.h>
/* #include <pci.h> no PCI on the S3C24XX */
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	 ...
	 */
	clk_power->epllcon = ((40 << 12) + (1 << 4) + 2);
	gpio->misccr |= 0x8;	/* 1 = use pads related USB for USB host */

	/*
	 * Enable USB host clock.
	 */
	clk_power->lockcon0 |= (1 << 4);
	...
	if (hc_reset(&gohci) < 0) {
		...
		clk_power->lockcon0 &= ~(1 << 4);//zhuchengzhi modfy 2016-06-26
		return -1;
	}
...
int usb_cpu_init(void)
{
	 ...
	 * "PLL value selection guide", 6-23, s3c2400_UM.pdf.
	 */
	writel((40 << 12) + (1 << 4) + 2, &clk_power->epllcon);
	 ...
	 * Enable USB host clock.
	 */
	writel(readl(&clk_power->lockcon0) | (1 << 4), &clk_power->lockcon0);
