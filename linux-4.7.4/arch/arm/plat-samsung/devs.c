...
//	[1] = DEFINE_RES_IRQ(IRQ_LCD_FIFO),
>>>>>>> 176aae0ce963f5803ac9cdc58973811606294c48
...
struct platform_device mys3c2416_device_lcd = {
	.name = "mys3c2416-lcd",
	.id	  = -1,
	.num_resources = ARRAY_SIZE(mys3c2416_lcd_resource),
	.resource = mys3c2416_lcd_resource,
	.dev = {
		.dma_mask = &samsung_device_dma_mask,
		.coherent_dma_mask = DMA_BIT_MASK(32),
	},
...
void __init mys3c2416_fb_set_platdata(struct mys3c2416fb_mach_info *pd)
{
	struct mys3c2416fb_mach_info *npd;

	npd = s3c_set_platdata(pd, sizeof(*npd), &mys3c2416_device_lcd);
	if (npd) {
...
void __init s3c24xx_fb_set_platdata(struct s3c2410fb_mach_info *pd)
{
	...
	if (npd) {
		npd->displays = kmemdup(pd->displays, sizeof(struct s3c2410fb_display) * npd->num_displays, GFP_KERNEL);
		if (!npd->displays)
